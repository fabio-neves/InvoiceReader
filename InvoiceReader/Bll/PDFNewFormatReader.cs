...
{
    public class PDFNewFormatReader: IFileChangedEventHandler
    {
        ...
        private DateTime GetVencimento(string page)
        {
            ...
            {
                var line = lines[idx];
                var isVencimentoField = line.Contains("vencimento:");
                if (isVencimentoField)
                {
                    var tokens = line.Split(' ');
                    DateTime.TryParse(tokens[3], culture, DateTimeStyles.None, out vencimento);
                }
            }
            return vencimento;
        }
        ...
        private List<InvoiceItem> ParsePage(string page, DateTime vencimento)
        {
            ...
            for (var idx = 0; idx < lines.Length; idx++)
            {
                ...
                var isItemFatura = tokens.Length > 1 && DateTime.TryParse(tokens[0], culture, DateTimeStyles.None, out data)
                    && decimal.TryParse(tokens[tokens.Length - 1], NumberStyles.Number, numberFormatInfo, out valorReal)
                    && decimal.TryParse(tokens[tokens.Length - 3], NumberStyles.Number, numberFormatInfo, out valorDolar);
